...
public:
	...
	*/
	...
	void buildChild(SegmentTreeNode* root) {
		if (root->start == root->end) return;
		int mid = (root->start + root->end) / 2;
		SegmentTreeNode* leftNode = new SegmentTreeNode(root->start, mid);
		SegmentTreeNode* rightNode = new SegmentTreeNode(mid + 1, root->end);
		root->left = leftNode;
		root->right = rightNode;
		buildChild(leftNode);
		buildChild(rightNode);
	}
